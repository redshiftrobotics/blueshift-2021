...
#include <pybind11/pybind11.h> // Kavi Dey
...
*/
	int buf_index = read_frame();
    struct buffer buf = buffers[buf_index];
	//unsigned char img = *((unsigned char *) buffers[buf_index].data);
    //printf(buff_index);
    //printf(img);
	//return &img;
    char image [buf.size];
...
// Kavi Dey
PYBIND11_MODULE(webcam,m)
{
    m.doc() = "Python bindings for v4l2 (Video for Linux 2)";

    py::class_<Webcam>(m,"Camera")
        .def( py::init( []( string path, int width, int height)
            {
                return new Webcam(path, width, height);
            }
            )
        )
    .def( "get_frame", &Webcam::frame, "A function to retrieve a frame of video from the camera");
}
